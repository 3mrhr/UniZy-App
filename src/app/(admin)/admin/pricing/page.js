...
import { DollarSign, ShieldAlert, Home, Truck, ShoppingBag, Utensils, Tag, Settings } from 'lucide-react';
import PricingManager from './PricingManager';

export default function GlobalPricingDashboard() {
    const [activeTab, setActiveTab] = useState('HOUSING');

    const tabs = [
        { id: 'HOUSING', label: 'Housing', icon: Home, color: 'bg-emerald-600' },
        { id: 'TRANSPORT', label: 'Transport', icon: ShoppingBag, color: 'bg-blue-600' }, // Reusing icon for visual placeholder
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">Super Admin dashboard controlling the centralized fee limits and rules for every UniZy module.</p>
                </div>
            ...
            {/* Active Module Pricing Manager */}
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                <PricingManager
                    moduleName={activeTab}
                    title={`${activeTabData.label} Commission & Fees`}
                    description={`Review and edit the global and zone-specific pricing tiers governing the ${activeTabData.label.toLowerCase()} system.`}
