    ...
    return (
        <div className="flex flex-col gap-6 h-full p-6 max-w-7xl mx-auto">
            <div className="flex justify-between items-start mb-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">SLA Configuration</h1>
                    <p className="text-slate-500 mt-1 font-medium">Define automated Time-to-Action Service Level Agreements.</p>
                </div>
                <div className="flex gap-3">
                    <Link href="/admin/sla/breaches" className="px-5 py-2.5 bg-red-50 text-red-600 hover:bg-red-100 rounded-xl font-bold transition-all flex items-center gap-2">
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8">
                {loading ? (
                    ...
                    <div className="py-20 text-center flex flex-col items-center">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 mb-2">No SLA Rules Found</h3>
                        <p className="text-slate-500 mb-6">Create your first rule to start monitoring operational performance.</p>
                        <button onClick={() => setShowModal(true)} className="px-6 py-3 bg-brand-50 text-brand-600 rounded-xl font-bold hover:bg-brand-100 transition-colors">
                            Configure Setup
                        </button>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {rules.map((rule) => (
                            <div key={rule.id} className={`p-6 rounded-3xl border-2 transition-all ${rule.active ? 'border-brand-50 bg-white' : 'border-slate-100 bg-slate-50 opacity-70'}`}>
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex items-center gap-3">
                                        <div className={`p-3 rounded-xl ${rule.active ? 'bg-brand-50' : 'bg-slate-200'}`}>
                                            {getModuleIcon(rule.module)}
                                        </div>
                                        <div>
                                            <span className="text-[10px] font-black tracking-widest uppercase text-slate-400">{rule.module}</span>
                                ...
                                <div className="my-6">
                                    <div className="flex items-baseline gap-2 mb-2">
                                        <span className="text-4xl font-black text-slate-900">{rule.thresholdMinutes}</span>
                                        <span className="text-sm font-bold text-slate-500">mins limit</span>
                                    </div>
                                    <div className="text-sm font-bold flex items-center gap-2">
                                        <span className="w-2 h-2 rounded-full bg-red-500"></span>
                                        Action target: <span className="text-slate-900">{rule.breachAction.replace('_', ' ')}</span>
                                    </div>
                                </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in p-4">
                    <form onSubmit={handleCreate} className="bg-white rounded-[2rem] p-8 w-full max-w-md shadow-2xl animate-fade-in-up">
                        <h2 className="text-2xl font-black text-slate-900 mb-6">New SLA Rule</h2>
